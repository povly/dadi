...
createChildClass('Selector', UiControl, {
  ...
  updatePlaceholder: function() {
    if (this.options && this.options.disablePlaceholder) {
      return;
    }
  ...
  handleKeyboardEventOutside: function(e) {
    ...
    switch(e.keyCode) {
      case KEY.RETURN:
        return false;
      case KEY.LEFT:
        for (let i = this._selectedItems.length - 1; i >= 0; i--) {
      ...
      if (this.dataURL) {
        ...
        if (this.options.hideSectionedChildren) {
          if (Selector.prototype.isClassEnabled(data[3] ? data[3] : '', 'section')) {
            ...
            for (let i = curPosition + 1; i < this.dataItems.length; i++) {
  ...
  addTagData: function(data) {
    ...
    for (let i in this._selectedItems) {
...
createChildClass('Indexer', UiUtil, {
  ...
  indexItem: function(k, v, additionalKey) {
    ...
    var index_key;
    var already_indexed = {};
    for (let i = 0; i < this.options.indexkeys.length; i++) {
    ...
    for (let i = 0; i < current_words.length; i++) {
      for (let j = 1; j <= this.options.chars; j++) {
  ...
  search: function(pattern) {
    ...
    each (self.storage.index[min_pattern.substr(0, self.options.chars)], function(k, v) {
      ...
      for (let i = 0; i < self.options.indexkeys.length; i++) {
